<?php
/*
Пример скрипта для сайта. Выводит последние одобренные комментарии к постам.
*/
$comments = $udb->get_rows("SELECT * FROM `".UC_PREFIX."comments` WHERE `approved` = '1' ORDER BY `date` DESC LIMIT 3"); //по умолчанию выводит только 3 комментария, но можно увеличить лимит
if(!$comments)
	...
	for($i = 0; $i < count($comments); $i++){
		...
		if(!$plain)
			...
			echo ' <b>'.$comments[$i]['author'].' (гость)</b> сказал:<br>';
